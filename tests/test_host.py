...
def test_fill_all_falls_through_to_cold_when_hot_idle(tmp_path):
...
def test_fill_all_respects_concurrency_cap(tmp_path):
    # max_concurrent_workers caps in-flight workers host-wide, independent of free slots/budget.
...
def test_fill_all_cold_starts_one_when_weighted_budget_affords_it(tmp_path):
    h = _host(tmp_path, max_slots=4)
    hot = FakeMgr("A", hot=True, nslots=1, will_take=0, project_id="p1")   # idle -> budget to cold
    cold = FakeMgr("C", hot=False, nslots=5, will_take=5, project_id="p2")
    h.managers = [hot, cold]
    h._weights = {"p1": 1.0, "p2": 2.0}   # C is a heavy (weight 2) cold project
    h._fill_all(now=None)
    # remaining = 4 - reserved_hot(1) = 3; weight 2 affords floor(3/2)=1; stagger caps to 1
    assert cold.fills == [1] and cold.busy_count() == 1


def test_fill_all_cold_blocked_by_weighted_budget(tmp_path):
    # The weighted RAM budget still gates cold provisioning under the stagger: a heavy cold
    # project can't start when hot warm pools already spend the whole budget, even with
    # concurrency headroom and the one-per-pass stagger budget free.
    h = _host(tmp_path, max_slots=2, max_concurrent_workers=5)
    hot = FakeMgr("A", hot=True, nslots=2, will_take=0, project_id="p1")   # reserves 2 units, idle
    cold = FakeMgr("C", hot=False, nslots=5, will_take=5, project_id="p2")
    h.managers = [hot, cold]
