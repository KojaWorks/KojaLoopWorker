...
def test_auth_failure_is_reclaim():
    # alive process, card still In progress, well under wallclock — but wedged at the
    # login prompt: reclaim now instead of holding the slot for the full cap.
...
def test_auth_failure_after_card_done_still_reaps():
    # card already left In progress takes precedence — don't misreport a finished worker
    # as an auth failure just because a stale 401 line sits in its scrollback.
