...
class Manager:
    ...
    def _build_prompt(self, slot: Slot, card, worker) -> str:
        ...
        return (
            ...
            f"Assignee, and STOP for a human merge decision.\n\n"
            f"WIND DOWN IN THIS ORDER — flipping the card OUT of 'In progress' is your EXIT "
            f"SIGNAL: the Manager starts reaping you the moment it sees that, so it must be "
            f"your LAST action; anything you do after it races the reaper. (1) On a clean "
            f"merge, set solved_in_pr and write a short summary at the bottom of the card "
            f"(keep Assignee = you). (2) Record one short env-feedback note — was anything "
            f"missing from your environment, did a gate block you wrongly, what slowed you "
            f"down? — in the project's Env-feedback table if one exists, else at the bottom of "
            f"the card. (3) LAST, set the card's status: Shipped for a clean merge; Needs "
            f"refinement or Backlog (and clear your Assignee) for a park. Then STOP — do not "
            f"pick another card; the Manager handles iteration.\n\n"
